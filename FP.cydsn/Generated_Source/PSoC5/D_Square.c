...
void D_Square_Start(void) 
{
    ...
    D_Square_CLKEN |= D_Square_CLKEN_MASK;
	D_Square_CLKSTBY |= D_Square_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: D_Square_Stop
********************************************************************************
*
* Summary:
...
void D_Square_Stop(void) 
{
    ...
    D_Square_CLKEN &= (uint8)(~D_Square_CLKEN_MASK);
	D_Square_CLKSTBY &= (uint8)(~D_Square_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: D_Square_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((D_Square_CLKEN & D_Square_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = D_Square_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = D_Square_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* D_Square__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(D_Square_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(D_Square_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: D_Square_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: D_Square_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = D_Square_GetSourceRegister();
    uint16 oldDivider = D_Square_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = D_Square_CLKEN & D_Square_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((D_Square_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: D_Square_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(D_Square_DIV_PTR);
}


/*******************************************************************************
* Function Name: D_Square_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    D_Square_MOD_SRC |= modeBitMask & (uint8)D_Square_MODE_MASK;
}


/*******************************************************************************
* Function Name: D_Square_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    D_Square_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(D_Square_MODE_MASK));
}


/*******************************************************************************
* Function Name: D_Square_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return D_Square_MOD_SRC & (uint8)(D_Square_MODE_MASK);
}


/*******************************************************************************
* Function Name: D_Square_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = D_Square_GetDividerRegister();
    uint8 oldSrc = D_Square_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        D_Square_MOD_SRC |= CYCLK_SSS;
        D_Square_MOD_SRC =
            (D_Square_MOD_SRC & (uint8)(~D_Square_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: D_Square_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return D_Square_MOD_SRC & D_Square_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: D_Square_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    D_Square_PHASE = clkPhase & D_Square_PHASE_MASK;
}


/*******************************************************************************
* Function Name: D_Square_GetPhase
********************************************************************************
*
* Summary:
...
{
    return D_Square_PHASE & D_Square_PHASE_MASK;
}
